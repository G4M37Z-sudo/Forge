    ...
    <div className="space-y-6">
      <ScrollReveal>
        <div className="space-y-2">
          <span className="inline-flex items-center px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider rounded bg-purple-500/10 text-purple-400 border border-purple-500/20">{toolData.category}</span>
      ...
      <ScrollReveal delay={100}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className={"rounded-xl bg-[#14141A] border overflow-hidden flex flex-col transition-colors " + (dragOver ? 'border-purple-500/50 bg-purple-500/5' : 'border-[#1E1E26]')} onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}>
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#1E1E26] bg-[#1E1E26]/50">
              ...
              <div className="flex items-center gap-2">
                ...
                {toolData.examples && toolData.examples.length > 0 && (
                  <button onClick={() => toolData.examples && setInput(toolData.examples[0].input)} className="text-xs text-purple-400/80 hover:text-purple-400 transition-colors">Example</button>
                )}
          ...
          <div className="rounded-xl bg-[#14141A] border border-[#1E1E26] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#1E1E26] bg-[#1E1E26]/50">
              <div className="flex items-center gap-2">
                <div className={"w-2 h-2 rounded-full " + (processing ? 'bg-purple-500 animate-pulse' : error ? 'bg-red-400' : output ? 'bg-emerald-400' : 'bg-[#5A5A6E]/30')} />
                <span className="text-xs font-medium text-[#A3A3B3] uppercase tracking-wider">Output</span>
                {processing && <span className="text-[10px] text-purple-400 animate-pulse">Processing...</span>}
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="p-4 rounded-xl bg-[#14141A] border border-[#1E1E26]">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="text-center p-2 rounded-lg bg-[#0B0B0F]/50">
                <p className="text-lg font-bold text-purple-400">{inputChars}</p>
              ...
              <div className="text-center p-2 rounded-lg bg-[#0B0B0F]/50">
                <p className="text-lg font-bold text-purple-400">{outputLines}</p>
        ...
        <ScrollReveal delay={200}>
          <div className="rounded-xl bg-[#14141A] border border-[#1E1E26] overflow-hidden">
            <button onClick={() => setShowExamples(!showExamples)} className="w-full flex items-center justify-between px-5 py-3.5 hover:bg-white/[0.02] transition-colors">
              ...
              <svg className={"w-3.5 h-3.5 text-purple-400 transition-transform " + (showExamples ? 'rotate-180' : '')} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </button>
            {showExamples && (
              <div className="px-5 pb-4 space-y-3 border-t border-[#1E1E26] pt-3">
                {toolData.examples.map((ex, i) => (
                  <div key={i} className="rounded-lg border border-[#1E1E26] bg-[#0B0B0F]/50 p-3 space-y-2">
                    ...
                    <button onClick={() => setInput(ex.input)} className="text-[10px] text-purple-400 hover:text-purple-300 transition-colors">Try this example</button>
                  </div>
                ))}
              </div>
            )}
